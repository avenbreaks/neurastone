...
import (
	...
	"github.com/avenbreaks/neurastone/tests/e2e/upgrade"
...
func (s *IntegrationTestSuite) upgrade(targetRepo, targetVersion string) {
	...
	// export node .neurd to local build/
	...
	node.SetCmd([]string{"neurd", "start", fmt.Sprintf("--chain-id=%s", s.upgradeParams.ChainID)})
	err = s.upgradeManager.RunNode(node)
