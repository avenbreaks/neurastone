...
KEY="mykey"
CHAINID="${CHAIN_ID:-neura_11235-1}"
...
# Set client config
neurad config keyring-backend "$KEYRING"
neurad config chain-id "$CHAINID"

# if $KEY exists it should be deleted
neurad keys add "$KEY" --keyring-backend $KEYRING --algo "$KEYALGO"

# Set moniker and chain-id for neura Network (Moniker can be anything, chain-id must be an integer)
neurad init "$MONIKER" --chain-id "$CHAINID"
...
node_address=$(neurad keys list | grep  "address: " | cut -c12-)
...
neurad add-genesis-account $KEY 100000000000000000000000000aISLM --keyring-backend $KEYRING
...
neurad gentx $KEY 1000000000000000000000aISLM --keyring-backend $KEYRING --chain-id "$CHAINID"
## In case you want to create multiple validators at genesis
## 1. Back to `neurad keys add` step, init more keys
## 2. Back to `neurad add-genesis-account` step, add balance for those
## 3. Clone this ~/.neurad home directory into some others, let's say `~/.clonedneurad`
## 4. Run `gentx` in each of those folders
## 5. Copy the `gentx-*` folders under `~/.clonedneurad/config/gentx/` folders into the original `~/.neurad/config/gentx`

# Collect genesis tx
neurad collect-gentxs

# Run this to ensure everything worked and that the genesis file is setup correctly
neurad validate-genesis

# Start the node (remove the --pruning=nothing flag if historical queries are not needed)
neurad start "$TRACE" --log_level $LOGLEVEL --minimum-gas-prices=0.0001aISLM --json-rpc.api eth,txpool,personal,net,debug,web3
