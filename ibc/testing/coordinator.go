...
// NewCoordinator initializes Coordinator with N EVM TestChain's (neura apps) and M Cosmos chains (Simulation Apps)
...
func SendMsgs(chain *ibctesting.TestChain, feeAmt int64, msgs ...sdk.Msg) (*sdk.Result, error) {
	...
	if neuraChain, ok := chain.App.(*app.neura); ok {
		bondDenom = neuraChain.StakingKeeper.BondDenom(chain.GetContext())
	} else {
