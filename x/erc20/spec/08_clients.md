...
Find below a list of  `neurad` commands added with the  `x/erc20` module. You can obtain the full list by using the `neurad -h` command. A CLI command can look like this:

```bash
neurad query erc20 params
```
...
```bash
neurad tx gov submit-proposal register-coin METADATA_FILE [flags]
```
...
```bash
neurad tx gov submit-proposal register-erc20 ERC20_ADDRESS... [flags]
```
...
```bash
neurad tx gov submit-proposal toggle-token-conversion TOKEN [flags]
```
...
```bash
neurad tx gov submit-proposal param-change PROPOSAL_FILE [flags]
```
