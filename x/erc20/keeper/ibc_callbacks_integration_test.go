...
var _ = Describe("Convert receiving IBC to Erc20", Ordered, func() {
	...
	haqqMeta := banktypes.Metadata{
		Description: "Base Denom for Haqq Network",
	...
	Describe("disabled params", func() {
		...
		It("should transfer and not convert to erc20", func() {
			...
			ibcOsmoBalanceBefore := s.app.BankKeeper.GetBalance(s.HaqqChain.GetContext(), receiverAcc, teststypes.UosmoIbcdenom)
			...
			ibcOsmoBalanceAfter := s.app.BankKeeper.GetBalance(s.HaqqChain.GetContext(), receiverAcc, teststypes.UosmoIbcdenom)
			s.Require().Equal(amount, ibcOsmoBalanceAfter.Amount.Int64())

			// check ERC20 balance - should be zero (no conversion)
			balanceERC20TokenAfter := s.app.Erc20Keeper.BalanceOf(s.HaqqChain.GetContext(), contracts.ERC20MinterBurnerDecimalsContract.ABI, pair.GetERC20Contract(), common.BytesToAddress(receiverAcc.Bytes()))
	...
	Describe("enabled params and registered uosmo", func() {
		...
		It("should transfer and convert uosmo to tokens", func() {
			...
			balanceTokenBefore := s.app.Erc20Keeper.BalanceOf(s.HaqqChain.GetContext(), contracts.ERC20MinterBurnerDecimalsContract.ABI, pair.GetERC20Contract(), common.BytesToAddress(receiverAcc.Bytes()))
		...
		It("should transfer and not convert unregistered coin (uatom)", func() {
			...
			// check balance before transfer is 0
			ibcAtomBalanceBefore := s.app.BankKeeper.GetBalance(s.HaqqChain.GetContext(), receiverAcc, teststypes.UatomIbcdenom)
			s.Require().Equal(int64(0), ibcAtomBalanceBefore.Amount.Int64())

			s.HaqqChain.Coordinator.CommitBlock()
			s.SendAndReceiveMessage(s.pathCosmosHaqq, s.IBCCosmosChain, "uatom", amount, sender, receiver, 1, "")

			// check balance after transfer
			ibcAtomBalanceAfter := s.app.BankKeeper.GetBalance(s.HaqqChain.GetContext(), receiverAcc, teststypes.UatomIbcdenom)
		...
		It("should transfer and not convert aISLM", func() {
			...
			pair, err := s.app.Erc20Keeper.RegisterCoin(s.HaqqChain.GetContext(), haqqMeta)
			s.Require().NoError(err)

			aislmInitialBalance := s.app.BankKeeper.GetBalance(s.HaqqChain.GetContext(), receiverAcc, utils.BaseDenom)

			// 1. Send aISLM from Haqq to Osmosis
			s.SendAndReceiveMessage(s.pathOsmosisHaqq, s.HaqqChain, utils.BaseDenom, amount, receiver, sender, 1, "")

			aislmAfterBalance := s.app.BankKeeper.GetBalance(s.HaqqChain.GetContext(), receiverAcc, utils.BaseDenom)
			...
			// 2. Send aISLM IBC coins from Osmosis to Haqq
			ibcCoinMeta := fmt.Sprintf("%s/%s", teststypes.AislmDenomtrace.Path, teststypes.AislmDenomtrace.BaseDenom)
			s.SendBackCoins(s.pathOsmosisHaqq, s.IBCOsmosisChain, teststypes.AislmIbcdenom, amount, sender, receiver, 1, ibcCoinMeta)
			...
			aislmFinalBalance := s.app.BankKeeper.GetBalance(s.HaqqChain.GetContext(), receiverAcc, utils.BaseDenom)
			...
			ibcCoinsBalance := s.app.BankKeeper.GetBalance(s.HaqqChain.GetContext(), receiverAcc, teststypes.AislmIbcdenom)
		...
		It("should transfer and convert original erc20", func() {
			...
			// 1. Send 'uosmo' from Osmosis to Haqq
			s.SendAndReceiveMessage(s.pathOsmosisHaqq, s.IBCOsmosisChain, "uosmo", amount, sender, receiver, 1, "")

			// validate 'uosmo' was transferred successfully and converted to ERC20
			balanceERC20Token := s.app.Erc20Keeper.BalanceOf(s.HaqqChain.GetContext(), contracts.ERC20MinterBurnerDecimalsContract.ABI, pair.GetERC20Contract(), common.BytesToAddress(receiverAcc.Bytes()))
			s.Require().Equal(amount, balanceERC20Token.Int64())

			// 2. Transfer back the erc20 from Haqq to Osmosis
			ibcCoinMeta := fmt.Sprintf("%s/%s", teststypes.UosmoDenomtrace.Path, teststypes.UosmoDenomtrace.BaseDenom)
			s.SendBackCoins(s.pathOsmosisHaqq, s.HaqqChain, types.ModuleName+"/"+pair.GetERC20Contract().String(), amount, receiver, sender, 1, ibcCoinMeta)
	...
	Describe("registered erc20", func() {
		...
		It("send native ERC-20 to osmosis, when sending back IBC coins should convert full balance back to erc20 token", func() {
			...
			// send back the IBC coins from Osmosis to Haqq
			s.SendAndReceiveMessage(s.pathOsmosisHaqq, s.IBCOsmosisChain, erc20Denomtrace.IBCDenom(), amount/2, receiver, sender, 1, erc20Denomtrace.GetFullDenomPath())
			// Check Balance
			balanceToken = s.app.Erc20Keeper.BalanceOf(s.HaqqChain.GetContext(), contracts.ERC20MinterBurnerDecimalsContract.ABI, pair.GetERC20Contract(), common.BytesToAddress(senderAcc.Bytes()))
			s.Require().Equal(amount, balanceToken.Int64())
...
var _ = Describe("Convert outgoing ERC20 to IBC", Ordered, func() {
	...
	Describe("registered erc20", func() {
		...
		It("should transfer available balance", func() {
			// Mint tokens and send to receiver
			_, err := s.app.Erc20Keeper.CallEVM(s.HaqqChain.GetContext(), contracts.ERC20MinterBurnerDecimalsContract.ABI, common.BytesToAddress(senderAcc.Bytes()), pair.GetERC20Contract(), true, "mint", common.BytesToAddress(senderAcc.Bytes()), big.NewInt(amount*2))
			s.Require().NoError(err)
			// Check Balance
			balanceToken := s.app.Erc20Keeper.BalanceOf(s.HaqqChain.GetContext(), contracts.ERC20MinterBurnerDecimalsContract.ABI, pair.GetERC20Contract(), common.BytesToAddress(senderAcc.Bytes()))
			s.Require().Equal(amount*2, balanceToken.Int64())
	...
	Describe("registered coin", func() {
		...
		It("should convert erc20 to ibc vouched and transfer", func() {
			...
			balance := s.app.BankKeeper.GetBalance(s.HaqqChain.GetContext(), senderAcc, teststypes.UosmoIbcdenom)
			...
			s.SendBackCoins(s.pathOsmosisHaqq, s.HaqqChain, pair.Denom, amount, sender, receiver, 1, teststypes.UosmoDenomtrace.GetFullDenomPath())
			s.IBCOsmosisChain.Coordinator.CommitBlock()
			// Check balance on the Osmosis chain
		...
		It("should timeout and reconvert coins", func() {
			...
			balanceERC20TokenAfter := s.app.Erc20Keeper.BalanceOf(s.HaqqChain.GetContext(), contracts.ERC20MinterBurnerDecimalsContract.ABI, pair.GetERC20Contract(), common.BytesToAddress(senderAcc.Bytes()))
			s.Require().Equal(int64(0), balanceERC20TokenAfter.Int64())
