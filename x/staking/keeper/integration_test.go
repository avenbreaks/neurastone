...
var _ = Describe("Staking module tests", func() {
	...
	Context("using a vesting account", func() {
		...
		BeforeEach(func() {
			...
			convMsg := vestingtypes.NewMsgConvertIntoVestingAccount(funder.AccAddr, vestingAccount.AccAddr, vestingStart, testutil.TestVestingSchedule.LockupPeriods, testutil.TestVestingSchedule.VestingPeriods, true, false, nil)
			res, err := tf.ExecuteCosmosTx(funder.Priv, factory.CosmosTxArgs{Msgs: []sdk.Msg{convMsg}})
			Expect(err).To(BeNil())
