...
SIMAPP = ./app
HTTPS_GIT := https://github.com/avenbreaks/neurastone.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=neura \
          -X github.com/cosmos/cosmos-sdk/version.AppName=$(NEURA_BINARY) \
...
build-docker:
	...
	$(DOCKER) rm neura || true
	# create a new container from the latest image
	$(DOCKER) create --name neura -t -i ${DOCKER_IMAGE}:latest neura
	# move the binaries to the ./build directory
	mkdir -p ./build/
	$(DOCKER) cp neura:/usr/bin/neurd ./build/
...
godocs:
	@echo "--> Wait a few seconds and visit http://localhost:6060/pkg/github.com/avenbreaks/neurastone/types"
	godoc -http=:6060
...
test-sim-custom-genesis-fast:
	...
	@echo "By default, ${HOME}/.$(NEURA_DIR)/config/genesis.json will be used."
	@go test -mod=readonly $(SIMAPP) -run TestFullAppSimulation -Genesis=${HOME}/.$(NEURA_DIR)/config/genesis.json \
...
test-sim-custom-genesis-multi-seed: runsim
	...
	@echo "By default, ${HOME}/.$(NEURA_DIR)/config/genesis.json will be used."
	@$(BINDIR)/runsim -Genesis=${HOME}/.$(NEURA_DIR)/config/genesis.json -SimAppPkg=$(SIMAPP) -ExitOnFail 400 5 TestFullAppSimulation
...
format: 
	...
	find . -name '*.go' -type f -not -path "./.devenv/*" -not -path "./vendor*" -not -path "*.git*" -not -path "./.direnv/*" -not -path "./client/docs/statik/statik.go" -not -name '*.pb.go' | xargs goimports -w -local github.com/avenbreaks/neurastone
...
PACKAGE_NAME:=github.com/avenbreaks/neurastone
