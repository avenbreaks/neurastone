...
import (
	...
	"github.com/avenbreaks/neurastone/precompiles/werc20/testdata"
...
var _ = Describe("WISLM Extension -", func() {
	var (
	...
	BeforeEach(func() {
		...
		WERC20ContractAddr, err = s.factory.DeployContract(
			...
			factory.ContractDeploymentData{
				Contract:        testdata.WISLMContract,
		...
		Expect(err).ToNot(HaveOccurred(), "failed to create wISLM extension")
		s.precompile = precompile

		err = s.network.App.EvmKeeper.AddEVMExtensions(s.network.GetContext(), precompile)
		Expect(err).ToNot(HaveOccurred(), "failed to add wISLM extension")
		...
		contractData = ContractData{
			...
			erc20ABI:       testdata.WISLMContract.ABI,
	...
	Context("WISLM specific functions", func() {
	...
	Context("Comparing to original WISLM contract", func() {
		BeforeEach(func() {
		...
		When("calling withdraw", func() {
			BeforeEach(func() {
				// Deposit into the WISLM contract to have something to withdraw
				depositCheck := passCheck.WithExpPass(true).WithExpEvents(werc20.EventTypeDeposit)
				txArgsContract, callArgsContract := s.getTxAndCallArgs(erc20Call, contractDataOriginal, werc20.DepositMethod)
				txArgsContract.Amount = amount
				txArgsContract.GasLimit = 50_000
	...
	Context("ERC20 specific functions", func() {
		When("querying name", func() {
			It("should return the correct name", func() {
				...
				Expect(err).ToNot(HaveOccurred(), "failed to unpack result")
				Expect(name).To(Equal("Islamic Coin"), "expected different name")
			})
		})
		...
		When("querying symbol", func() {
			It("should return the correct symbol", func() {
				...
				Expect(err).ToNot(HaveOccurred(), "failed to unpack result")
				Expect(symbol).To(Equal("ISLM"), "expected different symbol")
			})
		})
		...
		When("querying balance", func() {
			...
			It("should return a 0 balance new address", func() {
				// Query the balance
				txArgs, balancesArgs := s.getTxAndCallArgs(directCall, contractData, erc20.BalanceOfMethod, testutiltx.GenerateAddress())
		...
		When("querying allowance", func() {
			It("should return an existing allowance", func() {
				grantee := testutiltx.GenerateAddress()
				granter := sender
			...
			It("should return zero if no balance exists", func() {
				address := testutiltx.GenerateAddress()
