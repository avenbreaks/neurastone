...
func (s *PrecompileTestSuite) DoSetupTest() {
	...
	err = s.app.BankKeeper.MintCoins(s.ctx, coinomicstypes.ModuleName, coins)
	s.Require().NoError(err)
	err = s.app.BankKeeper.SendCoinsFromModuleToModule(s.ctx, coinomicstypes.ModuleName, authtypes.FeeCollectorName, inflCoins)
	s.Require().NoError(err)
	err = s.app.BankKeeper.SendCoinsFromModuleToModule(s.ctx, coinomicstypes.ModuleName, distrtypes.ModuleName, distrCoins)
	s.Require().NoError(err)
