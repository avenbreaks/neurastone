...
import (
	...
	auth "github.com/avenbreaks/neurastone/precompiles/authorization"
...
var (
	...
	validAttoTraceDenom = types.DenomTrace{Path: "channel-0", BaseDenom: "aISLM"}
	// validTraceDenomNoMicroAtto is a denomination trace with a valid IBC voucher name but no micro or atto prefix
	validTraceDenomNoMicroAtto = types.DenomTrace{Path: "channel-0", BaseDenom: "mISLM"}
...
func (s *PrecompileTestSuite) TestNameSymbol() {
	...
	testcases := []struct {
		name        string
		denom       string
		malleate    func(sdk.Context, *app.neura)
		expPass     bool
		errContains string
		expName     string
		expSymbol   string
		...
		},
		{
			...
			malleate: func(ctx sdk.Context, app *app.neura) {
		...
		},
		{
			...
			malleate: func(ctx sdk.Context, app *app.neura) {
				...
				err := app.BankKeeper.MintCoins(ctx, coinomicstypes.ModuleName, sdk.Coins{sdk.NewInt64Coin(validMetadata.Base, 1)})
...
func (s *PrecompileTestSuite) TestTotalSupply() {
	...
	testcases := []struct {
		name        string
		malleate    func(sdk.Context, *app.neura, *big.Int)
		expPass     bool
		errContains string
		expTotal    *big.Int
	}{
		...
		},
		{
			...
			malleate: func(ctx sdk.Context, app *app.neura, amount *big.Int) {
				// NOTE: we mint some coins to the inflation module address to be able to set denom metadata
				err := app.BankKeeper.MintCoins(ctx, coinomicstypes.ModuleName, sdk.Coins{sdk.NewCoin(validMetadata.Base, sdkmath.NewIntFromBigInt(amount))})
				s.Require().NoError(err)
			},
			expPass:  true,
...
func (s *PrecompileTestSuite) TestBalanceOf() {
	...
	testcases := []struct {
		name        string
		malleate    func(sdk.Context, *app.neura, *big.Int) []interface{}
		expPass     bool
		errContains string
		expBalance  *big.Int
	}{
		{
			...
			malleate: func(_ sdk.Context, _ *app.neura, _ *big.Int) []interface{} {
				return []interface{}{}
			},
		...
		{
			name: "pass - some coins",
			malleate: func(ctx sdk.Context, app *app.neura, amount *big.Int) []interface{} {
